...
def main():
    ...
    os.system("docker exec -i h1 iperf -u -s &")
    # 执行命令
    command = "docker exec -i h1 iperf -u -c 127.0.0.1 -b %dM -t 10"%args.bw
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
    time.sleep(11)
    information = "Send to 127.0.0.1\n"
    # 检查命令是否成功执行
    if result.returncode == 0:
      ...
      # 打印错误信息
       ...
       print(result.stderr)
    command = "docker exec -i h1 iperf -u -c 10.1.1.100 -b %dM -t 10"%args.bw
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
    time.sleep(11)
    information = "Send to 10.1.1.100\n"
    # 检查命令是否成功执行
    if result.returncode == 0:
       # 打印命令输出
       print("Command output:")
